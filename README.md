...
Geocoding results are cached in `geocode_cache.csv` (Location, Lat, Lon, Status), keyed by the exact raw `Location` string. `Status` is `Match`, `No Match`, or `Skipped` (mile-marker-only highway locations that were never sent to the geocoder).
